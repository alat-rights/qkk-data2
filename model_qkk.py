...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        self.debug["softmax(K.T)K"] = (
            (k.transpose(-1, -2).softmax(dim=-2) @ k)
            .detach()
            .cpu()
            .to(dtype=torch.float32)
        )
        self.debug["K.TK"] = (
            (k.transpose(-1, -2) @ k).detach().cpu().to(dtype=torch.float32)
        )
        self.debug["Q"] = q
        self.debug["K"] = k
        self.debug["normalized"] = (
            ((q @ k.transpose(-1, -2)).softmax(dim=-1) @ k)
            .softmax(dim=-2)
            .detach()
            .cpu()
        ...
        self.debug["softmax(Q,-1)softmax(K.T,-2)"] = (
            (q.softmax(-1) @ (k.transpose(-1, -2)).softmax(-2))
            .detach()
            .cpu()
            .to(dtype=torch.float32)
        )
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        parent_path = f"custom-QKK-weights-QKK-inference-2 (Now with scatterplots)" # f"test8-{time.time()}"
        os.mkdir(path=parent_path)
        for block_num, block in enumerate(self.transformer.h):

            def block_hook(module, input, output) -> None:
                raw_data = module.attn.debug
                ...
                data_dir = f"{parent_path}/block{block_num}-{time.time()}"
                os.mkdir(path=data_dir)
                for batch in range(
                    1
                ):  # Only using first batch, instead of x.shape[0] batches
                    batch_dir = f"{data_dir}/batch-{batch}"
                    os.mkdir(path=batch_dir)
                    plt.imshow(
                        X=x[batch].cpu(), cmap="viridis", interpolation="nearest"
                    )
                    plt.title("layer {block_num}, batch {batch}, input")
                    plt.savefig(f"{batch_dir}/{block_num}-{batch}-input.png")
                    for n_head in range(
                        1
                    ):  # Only using first head, instead of self.config.n_head heads:
                        head_dir = f"{batch_dir}/head-{n_head}"
                        os.mkdir(path=head_dir)

                        plt.imshow(
                            raw_data["softmax(Q)"][batch, n_head],
                            cmap="viridis",
                            interpolation="nearest",
                        )
                        plt.title(f"Softmax(Q, dim=-1) on a QKK model.\nlayer {block_num}, batch {batch}")
                        plt.xlabel("Head Size")
                        plt.ylabel("Sequence")
                        plt.savefig(
                            f"{head_dir}/{block_num}-{batch}-{n_head}-softmax(Q).png"
                        )
                        ...
                        plt.imshow(
                            raw_data["softmax(Q,-1)softmax(K.T,-2)"][batch, n_head],
                            cmap="viridis",
                            interpolation="nearest",
                        )
                        ...
                        plt.imshow(
                            raw_data["Q"][batch, n_head],
                            cmap="viridis",
                            interpolation="nearest",
                        )
                        plt.title(f"Q matrix on a QKK model.\nlayer {block_num}, batch {batch}")
                        plt.xlabel("Head Size")
                        plt.ylabel("Sequence")
                        plt.savefig(
                            f"{head_dir}/{block_num}-{batch}-{n_head}-Q.png"
                        )

                        plt.imshow(
                            raw_data["K"][batch, n_head],
                            cmap="viridis",
                            interpolation="nearest",
                        )
                        plt.title(f"K matrix on a QKK model.\nlayer {block_num}, batch {batch}")
                        plt.xlabel("Head Size")
                        plt.ylabel("Sequence")
                        plt.savefig(
                        ...
                        x_data, y_data = (
                            torch.log(raw_data["softmax(Q)"][batch, n_head].flatten()),
                            raw_data["scaled_softmax(QK.T)K"][batch, n_head].flatten(),
                        )
                        from scipy import stats

                        r = stats.pearsonr(x=x_data, y=y_data)[0]
                        print(f"Correlation: {r}")
                        fig = px.scatter(
                            x_data,
                            y_data,
                            title=f"x=flattened softmax(Q), y=flattened softmax(QK^T / sqrt(hs))K\nlayer {block_num}, batch {batch}, head {n_head}\nPearson's r = {r}",
                        )
                        fig.write_image(
                            f"{head_dir}/{block_num}-{batch}-smq-qkk-scatter.png-r={stats.pearsonr(x_data, y_data)[0]}.png"
                        )
